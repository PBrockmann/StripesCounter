...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.buttonShow = QPushButton('Show profil and peaks')
        self.buttonShow.setMaximumWidth(maximumWidth)
        self.buttonShow.clicked.connect(self.showProfilAndPeaks)
        ...
        layoutV2.addWidget(self.buttonShow)
    ...
    def on_motion(self, event):
        ...
        cont = False
        if event.inaxes == self.ax[0] and self.peaks != None:
            ...
            if len(self.listLabelPoints) < 2:
                ...
                if len(self.listLabelPoints) == 2:
                    ...
                    for p in list(self.ax[0].patches):
                        ...
                        xdata.append(cx)
                        ydata.append(cy)
                    self.line_object, = self.ax[0].plot(xdata, ydata, alpha=self.alpha_default, c='red', lw=self.profilLinewidth, picker=True)
    ...
    def drawProfil(self):
        ...
        try:
            ...
            for x in listEndSegment:
                self.ax[1].axvline(x=x, linestyle='dashed', color='gray', alpha=0.8)
    ...
    def update_lineWithWidthList(self):
        ...
        if self.profilLinewidth > 1:
            ...
            for i in range(len(xdata)-1):
                line = LineString([(xdata[i], ydata[i]), (xdata[i+1], ydata[i+1])])
                dilated = line.buffer(self.profilLinewidth/2., cap_style=2, join_style=1)
    ...
    def showProfilAndPeaks(self):
        ...
        self.ax[0].scatter(x, y, c='b', s=10, zorder=10)

        self.ax[1].clear()
        dList = [] 
        for p in points:   
            dList.append(points[0].distance(p) * self.scalePixel)
        wList = [] 
        for i in range(len(points)-1):   
            wList.append(points[i].distance(points[i+1]) * self.scalePixel)

        patchesList = []
        for i in range(len(points)-1):
            patchesList.append(patches.Rectangle([dList[i], 0], wList[i], 4))
        norm = plt.Normalize(min(wList), max(wList))
        orig_map=plt.cm.get_cmap('gray')
        reversed_map = orig_map.reversed()
        coll = matplotlib.collections.PatchCollection(patchesList, cmap=reversed_map, norm=norm)
        coll.set_array(np.array(wList))
        self.ax[1].add_collection(coll)

        self.ax[1].scatter(dList, [0 for i in dList], c='b', s=10, zorder=12)
        self.ax[1].set_ylim(-1,6)
        self.ax[1].set(yticklabels=[])  # remove the tick labels
        self.ax[1].tick_params(left=False)  # remove the ticks
        self.ax[1].grid(linestyle='dotted')
        self.ax[1].set_facecolor('whitesmoke')
